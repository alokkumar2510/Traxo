...
export default function EditCollectionModal({ isOpen, onClose, collectionItem, onSuccess }: EditCollectionModalProps) {
  ...
  // Prefill collection details when open
  useEffect(() => {
    if (isOpen && collectionItem) {
      setName(collectionItem.name);
      setDescription(collectionItem.description || "");
      setSelectedIcon(collectionItem.icon || "folder");
      setSelectedColor(collectionItem.color || "from-accent-primary to-accent-cyan");
      setError(null);
    }
  }, [isOpen, collectionItem]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await CollectionRepository.updateCollection(profile.id, collectionItem.id, updatedFields);
      handleClose();
    ...
    <Dialog isOpen={isOpen} onClose={handleClose}>
      <DialogContent className="max-w-[480px]" onClose={handleClose}>
        <DialogHeader>
          ...
          <DialogDescription>
            Modify folder titles, categories, icons, and gradients.
          </DialogDescription>
        </DialogHeader>
