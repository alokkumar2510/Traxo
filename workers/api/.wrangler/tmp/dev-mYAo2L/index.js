...
// .wrangler/tmp/bundle-sddvEj/strip-cf-connecting-ip-header.js
...
function decodeBase64UrlToString(str) {
  const base64 = str.replace(/-/g, "+").replace(/_/g, "/");
  return atob(base64);
}
__name(decodeBase64UrlToString, "decodeBase64UrlToString");
async function verifyFirebaseIdToken(token, projectId) {
  ...
  const header = JSON.parse(decodeBase64UrlToString(headerStr));
  const payload = JSON.parse(decodeBase64UrlToString(payloadStr));
  if (header.alg !== "RS256") {
      ...
      return c.json(
        {
          success: false,
          error: "Unauthorized",
          message: error3.message || "Invalid or expired token."
        },
        401
      );
    }
...
var errorHandler2 = /* @__PURE__ */ __name(() => {
  return (err, c) => {
...
function toFirestoreValue(val) {
  ...
  if (typeof val === "object") {
    if (val.seconds !== void 0 && val.nanoseconds !== void 0) {
      return { timestampValue: new Date(val.seconds * 1e3).toISOString() };
    }
    const fields = {};
...
__name(mapToFirestore, "mapToFirestore");
function mapFromFirestore(fields) {
  const obj = {};
  if (!fields)
    return obj;
  for (const [k, v] of Object.entries(fields)) {
...
__name(mapFromFirestore, "mapFromFirestore");
var FirestoreClient = class {
   ...
   */
  async createDocument(parentPath, docId, data) {
   ...
   */
  async runQuery(structuredQuery, parentPath) {
    const url = parentPath ? `${this.baseUrl}/${parentPath}:runQuery` : `${this.baseUrl}:runQuery`;
    const res = await fetch(url, {
      method: "POST",
      headers: this.getHeaders(),
...
// src/routes/trackers.ts
var trackersRouter = new Hono2();
var sectionConfigSchema = external_exports.object({
  selector: external_exports.string().min(1, "Selector is required"),
  selectorType: external_exports.enum(["css", "xpath"]),
  monitoredElement: external_exports.string().default("")
});
var jobConfigSchema = external_exports.object({
  role: external_exports.string().optional(),
  location: external_exports.string().optional(),
  remoteOnly: external_exports.boolean().default(false),
  minExperience: external_exports.number().optional(),
  maxExperience: external_exports.number().optional(),
  keywords: external_exports.array(external_exports.string()).default([])
});
var priceConfigSchema = external_exports.object({
  targetPrice: external_exports.number().optional(),
  currentPrice: external_exports.number().optional(),
  lowestPrice: external_exports.number().optional(),
  highestPrice: external_exports.number().optional(),
  currency: external_exports.string().default("INR")
});
var pdfConfigSchema = external_exports.object({
  fileName: external_exports.string().min(1, "File name is required")
});
var createTrackerSchema = external_exports.object({
  ...
  type: external_exports.enum(["website", "section", "job", "price", "pdf"]),
  frequency: external_exports.enum(["hourly", "6h", "12h", "daily"]),
...
var updateTrackerSchema = createTrackerSchema.partial();
trackersRouter.get("/", async (c) => {
  ...
  try {
    const list = await db.runQuery(query);
...
trackersRouter.get("/:id", async (c) => {
  ...
  const db = new FirestoreClient(c.env.FIREBASE_PROJECT_ID, c.req.header("Authorization").substring(7));
  try {
    const tracker = await db.getDocument(`trackers/${trackerId}`);
    if (!tracker) {
...
trackersRouter.post("/", async (c) => {
  ...
  try {
    ...
    const newTracker = {
      ...
      createdAt: now,
      updatedAt: now,
      sectionConfig: validated.type === "section" ? validated.sectionConfig : void 0,
      jobConfig: validated.type === "job" ? validated.jobConfig : void 0,
      priceConfig: validated.type === "price" ? validated.priceConfig : void 0,
      pdfConfig: validated.type === "pdf" ? validated.pdfConfig : void 0
    };
    const doc = await db.createDocument("trackers", trackerId, newTracker);
    if (c.env.SCANS_QUEUE) {
      await c.env.SCANS_QUEUE.send({ trackerId, userId });
    }
    return c.json({ success: true, tracker: doc }, 210);
  } catch (error3) {
    if (error3.name === "ZodError") {
      return c.json({ success: false, error: "Validation failed", details: error3.errors }, 400);
    }
    return c.json({ success: false, error: error3.message }, 500);
  }
});
...
var trackers_default = trackersRouter;

// src/routes/collections.ts
var collectionsRouter = new Hono2();
var collectionSchema = external_exports.object({
  name: external_exports.string().min(2, "Name must be at least 2 characters"),
  description: external_exports.string().optional(),
  color: external_exports.string().default("#3B82F6"),
  icon: external_exports.string().default("folder")
});
var associateTrackerSchema = external_exports.object({
  trackerId: external_exports.string().min(1, "Tracker ID is required")
});
collectionsRouter.get("/", async (c) => {
  ...
  try {
    const list = await db.runQuery(query, `users/${userId}`);
...
collectionsRouter.get("/:id", async (c) => {
  ...
  try {
    ...
    const collection = await db.getDocument(docPath);
    if (!collection) {
...
collectionsRouter.post("/", async (c) => {
  ...
  try {
    ...
    const doc = await db.createDocument(docPath, collectionId, newCollection);
    return c.json({ success: true, collection: doc }, 201);
  } catch (error3) {
    if (error3.name === "ZodError") {
      return c.json({ success: false, error: "Validation failed", details: error3.errors }, 400);
    }
...
collectionsRouter.delete("/:id", async (c) => {
  ...
  try {
    ...
    const collectionDoc = await db.getDocument(colPath);
    if (!collectionDoc) {
      return c.json({ success: false, error: "Collection not found" }, 404);
    }
    const trackersQuery = {
    ...
    const trackers = await db.runQuery(trackersQuery);
...
collectionsRouter.post("/:id/trackers", async (c) => {
  ...
  try {
    ...
    if (oldCollectionId) {
      ...
      const oldCol = await db.getDocument(oldColPath);
      if (oldCol) {
...
var collections_default = collectionsRouter;

// src/routes/notifications.ts
var notificationsRouter = new Hono2();
var createNotificationSchema = external_exports.object({
  trackerId: external_exports.string().min(1, "Tracker ID is required"),
  eventId: external_exports.string().min(1, "Event ID is required"),
  channel: external_exports.enum(["email", "telegram"]),
  status: external_exports.enum(["pending", "sent", "failed"]).default("pending")
});
...
notificationsRouter.post("/:id/read", async (c) => {
  ...
  try {
    ...
    const notification = await db.getDocument(docPath);
    if (!notification) {
...
notificationsRouter.post("/read-all", async (c) => {
  ...
  try {
    ...
    const unreadNotifications = await db.runQuery(query, `users/${userId}`);
...
notificationsRouter.post("/", async (c) => {
  ...
  try {
    ...
    const newNotification = {
      ...
      read: false,
      createdAt: now,
      sentAt: validated.status === "sent" ? now : void 0
    };
    const docPath = `users/${userId}/notifications`;
    const doc = await db.createDocument(docPath, notificationId, newNotification);
    return c.json({ success: true, notification: doc }, 201);
  } catch (error3) {
    if (error3.name === "ZodError") {
      return c.json({ success: false, error: "Validation failed", details: error3.errors }, 400);
    }
...
analyticsRouter.get("/summary", async (c) => {
  ...
  try {
    ...
    const stats = await db.getDocument(docPath);
    if (!stats) {
      return c.json({
        success: true,
        analytics: {
...
analyticsRouter.get("/trackers/:trackerId/metrics", async (c) => {
  ...
  try {
    ...
    const metrics = await db.getDocument(metricsPath);
    if (!metrics) {
      return c.json({
        success: true,
        metrics: {
...
// src/utils/queue.ts
async function processScanJob(trackerId, userId, env2, attempts) {
  console.log(`[Queue Consumer] Processing scan for tracker ${trackerId} (Attempt ${attempts})`);
  const url = `${env2.NEXT_JS_API_URL}/api/scan`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${env2.INTERNAL_API_SECRET}`,
      "Content-Type": "application/json"
    },
...
__name(processScanJob, "processScanJob");
...
async function handleFailedJob(trackerId, userId, env2, error3) {
  const errorMsg = error3?.message || String(error3);
  console.error(
...
__name(handleFailedJob, "handleFailedJob");

// src/utils/scheduler.ts
function mapCronToFrequency(cron) {
  switch (cron) {
    case "*/30 * * * *":
      return "30m";
    case "0 * * * *":
...
__name(mapCronToFrequency, "mapCronToFrequency");
async function handleCronTrigger(cronExpression, env2) {
  ...
  const scheduleUrl = `${env2.NEXT_JS_API_URL}/api/scan/schedule`;
  try {
    ...
    if (env2.SCANS_QUEUE) {
      ...
      for (let i = 0; i < trackers.length; i += chunkSize) {
        ...
        await env2.SCANS_QUEUE.sendBatch(chunk);
    ...
    console.error(`[Scheduler] Error running cron scheduler for ${frequency}:`, error3.message || error3);
    throw error3;
  }
}
__name(handleCronTrigger, "handleCronTrigger");
...
app.use(
  ...
  cors({
    origin: "*",
    // Adjust origin in production, e.g., "https://traxo.alokkumarsahu.in"
...
app.route("/api/trackers", trackers_default);
app.route("/api/collections", collections_default);
...
var src_default = {
        ...
        for (let i = 0; i < messages.length; i += concurrencyLimit) {
          ...
          await Promise.all(
            chunk.map(async (message) => {
              ...
              try {
                await processScanJob(trackerId, userId, env2, message.attempts);
              } catch (error3) {
                console.error(`[Queue Consumer] Error processing tracker ${trackerId}:`, error3.message || error3);
                ...
                if (nextAttempt < 4) {
                  ...
                  await handleFailedJob(trackerId, userId, env2, error3);
                  message.ack();
                }
              }
            })
  ...
  async scheduled(event, env2, ctx) {
    ctx.waitUntil(handleCronTrigger(event.cron, env2));
  }
};
...
// .wrangler/tmp/bundle-sddvEj/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-sddvEj/middleware-loader.entry.ts
